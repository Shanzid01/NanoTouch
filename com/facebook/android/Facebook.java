...
public class Facebook {
    ...
    class TokenRefreshConnectionHandler extends Handler {
        ...
        public void handleMessage(Message message) {
            ...
            TokenRefreshServiceConnection tokenRefreshServiceConnection = (TokenRefreshServiceConnection) this.connectionWeakReference.get();
            if (facebook != null && tokenRefreshServiceConnection != null) {
                ...
                if (string != null) {
                    ...
                    Session access$1 = facebook.session;
                    if (access$1 != null) {
                    ...
                    if (message.getData().containsKey(NativeProtocol.BRIDGE_ARG_ERROR_CODE)) {
                        tokenRefreshServiceConnection.serviceListener.onFacebookError(new FacebookError(string2, null, message.getData().getInt(NativeProtocol.BRIDGE_ARG_ERROR_CODE)));
                    } else {
    ...
    public void authorize(Activity activity, String[] strArr, int i, DialogListener dialogListener) {
        ...
        authorize(activity, strArr, i, sessionLoginBehavior, dialogListener);
    }
    ...
    private void authorize(Activity activity, String[] strArr, int i, SessionLoginBehavior sessionLoginBehavior, final DialogListener dialogListener) {
        ...
        OpenRequest permissions = new OpenRequest(activity).setCallback(new StatusCallback() {
            public void call(Session session, SessionState sessionState, Exception exception) {
                Facebook.this.onSessionCallback(session, sessionState, exception, dialogListener);
            }
        }).setLoginBehavior(sessionLoginBehavior).setRequestCode(i).setPermissions(Arrays.asList(this.pendingAuthorizationPermissions));
    ...
    private void onSessionCallback(Session session, SessionState sessionState, Exception exception, DialogListener dialogListener) {
        ...
        if (sessionState == SessionState.OPENED) {
            ...
            } else if ((exception instanceof FacebookAuthorizationException) && authorizationBundle != null && authorizationBundle.containsKey(Session.WEB_VIEW_ERROR_CODE_KEY) && authorizationBundle.containsKey(Session.WEB_VIEW_FAILING_URL_KEY)) {
                dialogListener.onError(new DialogError(exception.getMessage(), authorizationBundle.getInt(Session.WEB_VIEW_ERROR_CODE_KEY), authorizationBundle.getString(Session.WEB_VIEW_FAILING_URL_KEY)));
            } else {
